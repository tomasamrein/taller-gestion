...
import { Car, Wallet, TrendingUp, ArrowDownRight, ArrowUpRight, LifeBuoy, Activity, Wrench } from 'lucide-react'

export default function Dashboard() {
  const [stats, setStats] = useState({ total: 0, gananciaNeta: 0, ticketPromedio: 0, pendientes: 0 })
  const [chartData, setChartData] = useState([])
  const [brandData, setBrandData] = useState([])
  const [recentActivity, setRecentActivity] = useState([])

  const COLORS = ['#ea580c', '#f97316', '#fb923c', '#fdba74', '#94a3b8']

  // --- FUNCIÓN MÁGICA PARA ARREGLAR ZONA HORARIA ARGENTINA ---
  const fixDate = (dateString) => {
    if (!dateString) return new Date()
    // Si la fecha tiene 'T' (ej: 2026-01-25T14:30...) es timestamp completo, lo dejamos.
    if (dateString.includes('T')) return new Date(dateString)
    
    // Si es solo fecha (2026-01-25), le agregamos mediodía para evitar que al restar 3hs pase a ayer.
  ...
  const cargarDatos = async () => {
    ...
    const movimientosIngresos = ordenesFinalizadas.map(o => {
        ...
        return {
            id: `ing-${o.id}`,
            date: o.delivery_date || o.created_at, // Usamos fecha real
    ...
    // 3. MEZCLA Y ORDEN CRONOLÓGICO REAL
    const mix = [...movimientosIngresos, ...movimientosGastos].sort((a, b) => {
        // Convertimos ambas fechas de forma segura antes de comparar
    ...
    // 5. Marcas
    const marcasConteo = {}
    ordenesFinalizadas.forEach(o => {
        const marca = o.vehicles?.brand || 'Desconocido'
        marcasConteo[marca] = (marcasConteo[marca] || 0) + 1
    })
    const brandArray = Object.keys(marcasConteo).map(key => ({
        name: key, value: marcasConteo[key]
    ...
    const ticket = ordenesFinalizadas.length > 0 ? totalIngresos / ordenesFinalizadas.length : 0
    ...
    <div className="animate-fade-in space-y-6 pb-10 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p className="text-gray-500 text-sm lg:text-base">Métricas clave y estado actual.</p>
        </div>
        
        <div className="bg-slate-900 text-white px-5 py-3 rounded-xl shadow-lg flex items-center gap-4">
            <div className="bg-slate-800 p-2 rounded-lg"><Activity className="text-orange-500" /></div>
            <div>
                <p className="text-xs text-slate-400 font-bold uppercase">Ticket Promedio</p>
                <p className="text-xl font-bold">${Math.round(stats.ticketPromedio).toLocaleString()}</p>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-2 bg-white p-4 lg:p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="bg-white p-4 lg:p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center">
            <h3 className="font-bold text-lg text-gray-700 mb-2 w-full text-left border-l-4 border-slate-500 pl-3">Marcas Frecuentes</h3>
            {brandData.length > 0 ? (
                <div className="w-full h-[250px] relative">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={brandData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <span className="text-2xl font-bold text-gray-400 opacity-20"><Car size={40}/></span>
                    </div>
                </div>
            ) : (
                <div className="h-[250px] flex items-center justify-center text-gray-400 text-sm">Faltan datos</div>
            )}
        </div>
      </div>

      {/* TABLA DE MOVIMIENTOS ARREGLADA */}
                        ...
                        {recentActivity.map((mov) => (
                            <tr key={mov.id} className="hover:bg-orange-50 transition">
                                ...
                                <td className="p-4">
                                    ...
                                    {/* USAMOS fixDate PARA MOSTRAR LA FECHA CORRECTA */}
